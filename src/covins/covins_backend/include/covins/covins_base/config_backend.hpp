...
inline std::vector<VisColorRGB> LoadClassColors(std::string path)
{   
    std::vector<VisColorRGB> colors;
    if (estd2::GetValFromYaml<int>(path,"vis.showsemantics") == 0)
        return colors;     // semantics disabled 
    ...
    if (colorsNode.type() != cv::FileNode::SEQ) {
        std::cerr << "Invalid format in " << filename << std::endl;
        exit(EXIT_FAILURE);
    }

    for (const auto& colorNode : colorsNode) {
        int r = static_cast<int>(colorNode[0]);
        int g = static_cast<int>(colorNode[1]);
        int b = static_cast<int>(colorNode[2]);
...
inline std::vector<std::string> LoadClassLabels(std::string path)
{
    std::vector<std::string> labels;
    if (estd2::GetValFromYaml<int>(path,"vis.showsemantics") == 0)
        return labels;     // semantics disabled 
    ...
    // Read labels and colors from the file
    ...
    for (const auto& labelNode : labelsNode) {
        std::string label;
        labelNode >> label;
...
inline std::vector<int> LoadClassCategoryIds(std::string path, std::string key)
{
    std::vector<int> ids;
    if (estd2::GetValFromYaml<int>(path,"vis.showsemantics") == 0)
...
namespace ade20k {
    const std::vector<VisColorRGB> class_colors = LoadClassColors(conf);
    const std::vector<std::string> class_labels = LoadClassLabels(conf);
    const std::vector<int> obstacle_ids = LoadClassCategoryIds(conf, "obstacle_ids");
    const std::vector<int> floor_ceil_ids = LoadClassCategoryIds(conf, "floor_ceil_ids");
    const std::vector<int> dynamic_ids = LoadClassCategoryIds(conf, "dynamic_ids");
}
