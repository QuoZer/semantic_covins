...
auto KeyframeDatabase::DetectCandidates(KeyframePtr kf, precision_t min_score)->KeyframeVector {
    ...
    ///TODO: This mutex seams to be closed for an awfully lot of time
    {
    ...
    if(lKFsSharingWords.empty()){
        std::cout << "DetectCandidates returned zero candidates: either no KF or all keyframes in local map" << std::endl;
    ...
    if(lScoreAndMatch.empty()){
        std::cout << "DetectCandidates returned zero candidates: no KF-s above minscore" << std::endl;
        return KeyframeVector();
    }
    ...
    if (vpLoopCandidates.empty())
        std::cout << "DetectCandidates returned zero candidates: all removed at last check" << std::endl;

    std::cout << "DetectCandidates returned " << vpLoopCandidates.size() << " candidates" << std::endl;
