...
PlaceRecognition::PlaceRecognition(ManagerPtr man, int client_id, bool perform_pgo)
    : mapmanager_(man),
      perform_pgo_(perform_pgo),
      voc_(mapmanager_->GetVoc()),
      mnCovisibilityConsistencyTh(covins_params::placerec::cov_consistency_thres),
...
auto PlaceRecognition::CorrectLoop()->bool {
    std::stringstream ss;
    ss << "\033[1;32m+++ PLACE RECOGNITION FOUND by agent"<< client_id_ <<" +++\033[0m";
    cout << ss.str() << endl;
...
auto PlaceRecognition::DetectLoop()->bool {
    int kf_buffer_size = 0;
    {
        std::unique_lock<std::mutex> lock(mtx_in_);
        kf_query_ = buffer_kfs_in_.front();
        buffer_kfs_in_.pop_front();
        kf_query_->SetNotErase();
        kf_buffer_size = buffer_kfs_in_.size();
    }
    std::cout <<"Agent" << this->client_id_ << " KF buffer size: " << kf_buffer_size <<std::endl;
    ...
    if(kf_query_->id_.first < covins_params::placerec::start_after_kf) {
        kf_query_->SetErase();
        std::cout<<"DetectLoop exited at start_after_kf"<<std::endl;
        return false;
    }
    ...
    if(last_loops_.count(kf_query_->id_.second)) {
        if((kf_query_->id_.first - last_loops_[kf_query_->id_.second]) < covins_params::placerec::consecutive_loop_dist) {
            kf_query_->SetErase();
            std::cout<<"DetectLoop exited at consecutive_loop_dist"<<std::endl;
            return false;
        }
    }
    ...
    if (vpCandidateKFs.empty()) {
        ...
        kf_query_->SetErase();
        std::cout<<"DetectLoop exited at no loop candidates"<<std::endl;
        return false;
    }
    ...
    /// TODO: understand this part, because we die here
    ...
    if (mvpEnoughConsistentCandidates.empty()) {
        kf_query_->SetErase();
        std::cout<<"DetectLoop exited at not consistent with previous loop candidates"<<std::endl;
        return false;
    } else {
        return true;
    }
    ...
    while(1){
        if (CheckBuffer()) {
            ...
            if(detected) {
                ...
                if(found_se3) {
                    this->CorrectLoop();
                    std::cout << "\033[1;32m+++ Loop detected and corrected +++\033[0m" << endl;
                }
            }
