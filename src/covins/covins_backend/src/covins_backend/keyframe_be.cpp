                   ...
                   msg.img_dim_x_min,msg.img_dim_y_min,msg.img_dim_x_max,msg.img_dim_y_max)
{
    ...
    keypoint_classes = msg.keypoint_classes;
    ...
    if(keypoint_classes.empty()) {
        std::cout << COUTWARN << "No semantic labels in the message with KF " << msg.id << std::endl;
    }
    // else {
    //     std::cout << COUTNOTICE << "There are " << keypoint_classes.size() << " classes in the message \n";
    // }
...
auto Keyframe::ConvertToMsgFileExport(MsgKeyframe &msg)->void {
    ...
    msg.keypoint_classes = keypoint_classes; 
