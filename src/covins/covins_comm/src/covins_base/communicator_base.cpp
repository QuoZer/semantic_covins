...
auto CommunicatorBase::RecvAll(unsigned int sz, std::vector<char> &buffer)->int {
    ...
    while (tot_bytes < sz) {
        ...
        if (n_bytes <= 0) {
            std::cout << "|RecvA| Num bytes: " << n_bytes << "; tot_bytes: " << tot_bytes <<"; buf size: " << buffer.size() << "\n" ;
            if (n_bytes == 0) {
                // connection closed
...
auto CommunicatorBase::RecvAll(unsigned int sz, MsgInfoType &buffer)->int {
    ...
    while(tot_bytes < buf_stream.size()) {   // 200 bytes
        n_bytes = recv(newfd_, &buf_stream[tot_bytes], bytes_left, 0);
        if (n_bytes <= 0) {
            // std::cout << "|RecvAt| Num bytes: " << n_bytes << "; tot_bytes: " << tot_bytes << "; bytes_left: " << bytes_left <<"; buf size: " << buf_stream.size() << "\n" ;
            if (n_bytes == 0) {
                // connection closed
                printf("------- selectserver: socket %d hung up. -------\n", newfd_);
            } else {
    ...
    // std::cout << "|RecvAt| Received a msg with info of size: " << buffer.size() << "\n";
...
auto CommunicatorBase::RecvMsg()->void {
    ...
    while(true) {
        //receive msg_type_
        unsigned int sz = msg_type_container_.size()*sizeof(msg_type_container_[0]);
        if(RecvAll(sz, msg_type_container_) == -1) {
            printf("----> Exit Recv Thread in msg type check\n");
            SetFinish();
            break;
        }
        ...
        //receive message
        // printf("Expected msg size 2: %d \n", total_msg);
        if(RecvAll(total_msg, recv_buf_) == -1) {
            printf("----> Exit Recv Thread on full msg\n");
            SetFinish();
            break;
        }
...
auto CommunicatorBase::SendMsgContainer(message_container &msg)->void {
    // std::cout << "|CommB| Sending msg with msg info of zise: " << msg.msg_info.size() << "\n";
    // for (int i = 0; i < msg.msg_info.size(); i++)
    //     std::cout << msg.msg_info[i] << ", ";
    // std::cout << "\n";
...
auto CommunicatorBase::Serialize(MsgKeyframe &msg)->void {
    ...
    // int max_class = *std::max_element(msg.keypoint_classes.begin(), msg.keypoint_classes.end());
    // std::cout << "|CommB| Sending kf " << msg.id << " with class vector size: " << msg.keypoint_classes.size() << " with maximum detected class: " << max_class << " and classes: \n";
    // int last_class = 0;
    // for (int i = 0; i < msg.keypoint_classes.size(); i++) {
    //     if (msg.keypoint_classes[i] == last_class || msg.keypoint_classes[i] == 0) continue;
    //     std::cout << static_cast<int>(msg.keypoint_classes[i]) << ", ";
    //     last_class = msg.keypoint_classes[i];
    // }
    // std::cout << "\n";
    ...
    // printf("Serialized kf msg length: %d \n", package_size_send_);
