...
#include "../../covins_backend/thirdparty/DBoW2/DBoW2/FeatureVector.h"
    ...
    while(f1it!=f1end && f2it!=f2end)
    {
        if(f1it->first == f2it->first)
        {
            for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
            {
                ...
                ///////// SEMANTIC CHECK //////////
                int semantic1 = kp1.class_id;  
                // if(semantic1 == 12) continue;    // label for the 'person' class     
                if (std::find(covins_params::orb::dynamic_ids.begin(), covins_params::orb::dynamic_ids.end(), semantic1) != covins_params::orb::dynamic_ids.end()) 
                            continue; 
                ...
                for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                {
                    ...
                    ///////// SEMANTIC CHECK //////////
                    int semantic2 = kp2.class_id;  
                    // if(semantic2 == 12) continue;    // label for a 'person' class     
                    if (std::find(covins_params::orb::dynamic_ids.begin(), covins_params::orb::dynamic_ids.end(), semantic2) != covins_params::orb::dynamic_ids.end()) 
                            continue; 
        ...
        while(f1it!=f1end && f2it!=f2end)
        {
            if(f1it->first == f2it->first)
            {
                for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
                {
                    ...
                    ///////// SEMANTIC CHECK //////////
                    int semantic1 = kp1.class_id;  
                    // if(semantic1 == 12) continue;    // label for a 'person' class     
                    if (std::find(covins_params::orb::dynamic_ids.begin(), covins_params::orb::dynamic_ids.end(), semantic1) != covins_params::orb::dynamic_ids.end()) 
                            continue; 
