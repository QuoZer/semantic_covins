...
#include <covins/covins_base/config_comm.hpp> // params 
    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
{
    ...
    else if(mSensor==IMU_MONO_SEMANTIC)
        cout << "Monocular-Inertial with Semantic Labels" << endl;
    ...
    // if (mSensor == IMU_MONO_SEMANTIC) {
    //     //TODO: Load semantic dataset
    //     // dynamic_ids = LoadCategoryIds(covins_params::orb::dataset_path, "dynamic_ids");
    ...
    if (mSensor==IMU_STEREO || mSensor==IMU_MONOCULAR || mSensor==IMU_MONO_SEMANTIC)
    ...
    mpLocalMapper = new LocalMapping(this, mpAtlas, mSensor==MONOCULAR || mSensor==IMU_MONOCULAR || mSensor==IMU_MONO_SEMANTIC, mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO || mSensor==IMU_MONO_SEMANTIC, strSequence);
...
cv::Mat System::TrackSemantic(const cv::Mat &im,const cv::Mat &mask, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    if(mSensor!=IMU_MONO_SEMANTIC)
    {
        cerr << "ERROR: you called TrackMonocular but input sensor was not set to Monocular nor Monocular-Inertial." << endl;
    ...
    // std::cout << "|S| Grabbing.. image" << std::endl;
    cv::Mat Tcw = mpTracker->GrabImageMonoSemantic(im, mask,timestamp,filename);
...
void System::SaveTrajectoryEuRoC(const string &filename)
{
    ...
    if (mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO  || mSensor==IMU_MONO_SEMANTIC)
