     ...
     mpCamera2(nullptr)
{
    ...
    std::chrono::steady_clock::time_point time_StartExtORB = std::chrono::steady_clock::now();
#endif
    // std::cout<< "|F| Extracting features..." <<std::endl;
    ExtractORB(0,imGray,0,1000);
#ifdef REGISTER_TIMES
    std::chrono::steady_clock::time_point time_EndExtORB = std::chrono::steady_clock::now();
    ...
    // std::cout<< "|F| Assigning class labels..." <<std::endl;
    AssignClass(mask);
...
void Frame::AssignClass(const cv::Mat& mask)
{
    for(int i=0; i<N; i++)
    {
        int classLabel = mask.at<uint8_t>(mvKeys[i].pt);
        mvKeys[i].class_id = classLabel;
    }
}
