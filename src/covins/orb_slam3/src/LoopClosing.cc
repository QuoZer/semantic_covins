    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(bDetected)
            {
                if(mbMergeDetected)
                {
                    if ((mpTracker->mSensor==System::IMU_MONOCULAR || mpTracker->mSensor==System::IMU_MONO_SEMANTIC || mpTracker->mSensor==System::IMU_STEREO) &&
                                                 ...
                                                 std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    if(nNumProjMatches >= nProjMatches)
    {
        ...
        if( (mpTracker->mSensor==System::IMU_MONOCULAR || mpTracker->mSensor==System::IMU_MONO_SEMANTIC) && !pCurrentKF->GetMap()->GetIniertialBA2())
            bFixedScale=false;
                                             ...
                                             int &nNumCoincidences, std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    for(KeyFrame* pKFi : vpBowCand)
    {
        ...
        if(!bAbortByNearKF && numBoWMatches >= nBoWMatches) // TODO pick a good threshold
        {
            ...
            if( (mpTracker->mSensor==System::IMU_MONOCULAR || mpTracker->mSensor==System::IMU_MONO_SEMANTIC) && !mpCurrentKF->GetMap()->GetIniertialBA2())
                bFixedScale=false;
...
void LoopClosing::MergeLocal2()
{
    ...
    if((mpTracker->mSensor==System::IMU_MONOCULAR || mpTracker->mSensor==System::IMU_MONO_SEMANTIC || mpTracker->mSensor==System::IMU_STEREO)&& !pCurrentMap->GetIniertialBA2()){
