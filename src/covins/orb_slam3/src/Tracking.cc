    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr)
{
    ...
    if(sensor==System::IMU_MONO_SEMANTIC || sensor==System::IMU_MONOCULAR || sensor==System::IMU_STEREO)
    {
...
bool Tracking::ParseORBParamFile(cv::FileStorage &fSettings)
{
    ...
    if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR || mSensor==System::IMU_MONO_SEMANTIC)
...
cv::Mat Tracking::GrabImageMonoSemantic(const cv::Mat &im, const cv::Mat &mask, const double &timestamp, string filename)
{
    mImGray = im;
    mImMask = mask.clone();
    ...
    if(mSensor == System::IMU_MONO_SEMANTIC)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
        {
    ...
    // std::cout << "|T| Saved frame" << std::endl;
    lastID = mCurrentFrame.mnId;
    Track();
...
void Tracking::Track()
{
    ...
    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO|| mSensor==System::IMU_MONO_SEMANTIC) && mpLastKeyFrame)
    ...
    if ((mSensor == System::IMU_MONOCULAR|| mSensor==System::IMU_MONO_SEMANTIC || mSensor == System::IMU_STEREO) && !mbCreatedMap)
    {
#ifdef REGISTER_TIMES
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                if (!bOK)
                {
                    if ( mCurrentFrame.mnId<=(mnLastRelocFrameId+mnFramesToResetIMU) &&
                         (mSensor==System::IMU_MONOCULAR || mSensor==System::IMU_MONO_SEMANTIC || mSensor==System::IMU_STEREO))
                    {
                        mState = LOST;
                    }
            ...
            else
            {
                ...
                if (mState == RECENTLY_LOST)
                {
                    ...
                    bOK = true;
                    if((mSensor == System::IMU_MONOCULAR || mSensor==System::IMU_MONO_SEMANTIC || mSensor == System::IMU_STEREO))
                    {
        ...
        if((mCurrentFrame.mnId<(mnLastRelocFrameId+mnFramesToResetIMU)) && (mCurrentFrame.mnId > mnFramesToResetIMU) && 
            ((mSensor == System::IMU_MONOCULAR)|| mSensor==System::IMU_MONO_SEMANTIC || (mSensor == System::IMU_STEREO)) && pCurrentMap->isImuInitialized())
        {
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
            ...
            if(bNeedKF && (bOK|| (mState==RECENTLY_LOST && (mSensor == System::IMU_MONOCULAR || mSensor==System::IMU_MONO_SEMANTIC || mSensor == System::IMU_STEREO))))
                CreateNewKeyFrame();
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mpInitializer)
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            if (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_MONO_SEMANTIC)
            {
                if(mpImuPreintegratedFromLastKF)
                {
                    delete mpImuPreintegratedFromLastKF;
    ...
    else
    {
        ...
        ((mSensor == System::IMU_MONOCULAR || mSensor==System::IMU_MONO_SEMANTIC)&&(mLastFrame.mTimeStamp-mInitialFrame.mTimeStamp>1.0)))
        {
            delete mpInitializer;
...
void Tracking::CreateMapInAtlas()
{
    ...
    if (mSensor==System::IMU_STEREO || mSensor == System::IMU_MONOCULAR || mSensor==System::IMU_MONO_SEMANTIC)
    ...
    if((mSensor == System::IMU_MONOCULAR  || mSensor==System::IMU_MONO_SEMANTIC || mSensor == System::IMU_STEREO ) && mpImuPreintegratedFromLastKF)
    {
        delete mpImuPreintegratedFromLastKF;
        mpImuPreintegratedFromLastKF = new IMU::Preintegrated(IMU::Bias(),*mpImuCalib);
    }
...
void Tracking::UpdateLastFrame()
{
    ...
    if(mnLastKeyFrameId==mLastFrame.mnId || mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR || mSensor==System::IMU_MONO_SEMANTIC || !mbOnlyTracking)
        return;
...
bool Tracking::TrackWithMotionModel()
{
    ...
    int nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,th,mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR || mSensor==System::IMU_MONO_SEMANTIC);
    ...
    if(nmatches<20)
    {
        ...
        nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,2*th,mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR || mSensor==System::IMU_MONO_SEMANTIC);
...
{
    if(((mSensor == System::IMU_MONOCULAR || mSensor==System::IMU_MONO_SEMANTIC) || (mSensor == System::IMU_STEREO)) && !mpAtlas->GetCurrentMap()->isImuInitialized())
    {
        if ((mSensor == System::IMU_MONOCULAR || mSensor==System::IMU_MONO_SEMANTIC) && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=0.25)
            return true;
    ...
    const bool c1c = mSensor!=System::MONOCULAR && mSensor!=System::IMU_MONOCULAR && mSensor!=System::IMU_MONO_SEMANTIC && mSensor!=System::IMU_STEREO && (mnMatchesInliers<nRefMatches*0.25 || bNeedToInsertClose) ;
    ...
    bool c4 = false;
    if ((((mnMatchesInliers<75) && (mnMatchesInliers>15)) || mState==RECENTLY_LOST) && ((mSensor == System::IMU_MONOCULAR || mSensor==System::IMU_MONO_SEMANTIC))) // MODIFICATION_2, originally ((((mnMatchesInliers<75) && (mnMatchesInliers>15)) || mState==RECENTLY_LOST) && ((mSensor == System::IMU_MONOCULAR)))
        c4=true;
    else
        c4=false;
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if(mSensor!=System::MONOCULAR && mSensor != System::IMU_MONOCULAR && mSensor!=System::IMU_MONO_SEMANTIC) // TODO check if incluide imu_stereo
    {
...
void Tracking::SearchLocalPoints()
{
    ...
    if(nToMatch>0)
    {
        ...
        else if(!mpAtlas->isImuInitialized() && (mSensor==System::IMU_MONOCULAR || mSensor==System::IMU_MONO_SEMANTIC || mSensor==System::IMU_STEREO))
        {
            th=10;
        }
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    if((mSensor == System::IMU_MONOCULAR  || mSensor==System::IMU_MONO_SEMANTIC || mSensor == System::IMU_STEREO) &&mvpLocalKeyFrames.size()<80)
    {
