...
void MapDrawer::DrawMapPoints()
{
    ...
    for(size_t i=0, iend=vpMPs.size(); i<iend;i++)
    {
        ...
        /////
        // std::map<ORB_SLAM3::KeyFrame *, std::tuple<int, int>> kf_obs = vpMPs[i]->GetObservations();
        // std::map<int, int> class_dest;  // [class|n_observations] destribution of observation classes
        // for (auto observation = kf_obs.begin(); observation != kf_obs.end(); ++observation) {
        //     KeyFrame* obs_kf = observation->first;
            
        //     int feat_idx = lm_p->GetFeatureIndex(obs_kf);
        //     // Increment a class probability
        //     class_dest[obs_kf->keypoint_classes[feat_idx]]++;
        // }
        // // Finding max value
