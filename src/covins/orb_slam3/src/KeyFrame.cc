...
auto KeyFrame::ConvertToMsg(covins::MsgKeyframe &msg, KeyFrame *kf_ref, bool is_update, size_t cliend_id)->void {
    ...
    if(!is_update) {
        ...
        msg.keypoint_classes = this->keys_class_;
    }
    ...
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.mTrl), mnNumberOfOpt(0)
{
    ...
    #ifdef COVINS_MOD
    {
        ...
        keys_class_.reserve(mvKeys.size());
        ...
        for(const auto &i : mvKeysUn){
            ...
            keys_class_.push_back(i.class_id);
            // if (i.class_id == last_class || i.class_id == 0) continue;
            // std::cout << i.class_id << ", ";
            // last_class = i.class_id;
        }
        // std::cout << "\n";
