...
public:
    ...
    // Proccess the given monocular frame, semantic mask and optionally imu data
    // Input images: RGB (CV_8UC3) or grayscale (CV_8U). RGB is converted to grayscale.
    // Returns the camera pose (empty if tracking fails).
    cv::Mat TrackSemantic(const cv::Mat &im, const cv::Mat &mask , const double &timestamp, const vector<IMU::Point>& vImuMeas = vector<IMU::Point>(), string filename="");
    ...
    // ids of the dynamic semantic classes to mask 
